...
var bigram = function (builder) {
  ...
  _lunr.default.Pipeline.registerFunction(pipelineFunction, 'bigram'); // Add the pipeline function to both the indexing pipeline and the
  // searching pipeline


  builder.pipeline.before(_lunr.default.stemmer, pipelineFunction);
  builder.searchPipeline.before(_lunr.default.stemmer, pipelineFunction);
};
...
var metadataUpdate = function (builder) {
  ...
  _lunr.default.Pipeline.registerFunction(pipelineFunction, 'metadataUpdate'); // Add the pipeline function to the indexing pipeline


  builder.pipeline.before(_lunr.default.stemmer, pipelineFunction); // Whitelist the tokenLength metadata key
...
let idx = (0, _lunr.default)(function () {
  ...
  this.pipeline.reset(); // this.searchPipeline.reset();
  // similarity tuning

  this.k1(0.2);
  this.b(1);
  this.use(metadataUpdate); // this.use(bigram);
